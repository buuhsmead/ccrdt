...
import com.github.mboogerd.ccrdt.lattice._
...
object LexProductInstances extends LexProductSyntax {
  ...
  /**
    ...
    */
  class LexProductEq[S: Eq, T: Eq] extends Eq[S :⊠: T] {
    override def eqv(x: S :⊠: T, y: S :⊠: T): Boolean = {
      val leftEq = implicitly[Eq[S]].eqv(x.left, y.left)
      lazy val rightEq = implicitly[Eq[T]].eqv(x.right, y.right)
      leftEq && rightEq
    }
  }
  ...
  def defaultJoin[S: JoinSemilattice: Eq, T: JoinSemilattice: Eq](lhs: S :⊠: T, rhs: S :⊠: T): S :⊠: T = {
    val sOrder: PartialOrder[S] = implicitly[JoinSemilattice[S]].joinPartialOrder

    sOrder.partialCompare(lhs.left, rhs.left) match {
      case -1 ⇒ rhs
      case 1 ⇒ lhs
      case 0 ⇒ lhs // they are equivalent, so either of the is fine
  ...
  class LexProductChain[S: Chain, T: Chain] extends Chain[S :⊠: T] {
    override def compare(x: S :⊠: T, y: S :⊠: T): Int = {
      math.signum(implicitly[Order[S]].compare(x.left, y.left)) match {
        case 0 ⇒ implicitly[Order[T]].compare(x.right, y.right)
        case x ⇒ x
      }
    }
