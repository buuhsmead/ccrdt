...
class LatticeOps[S: JoinSemilattice](lat1: S) {
  protected val sJSL = implicitly[JoinSemilattice[S]]
  def join[T <: S](lat2: T): S = sJSL.join(lat1, lat2)
  def ⊔[T <: S](lat2: T): S = join(lat2)
}

final class BoundedLatticeOps[S: BoundedJoinSemilattice](lat1: S) extends LatticeOps[S](lat1) {
  override protected val sJSL = implicitly[BoundedJoinSemilattice[S]]
  def bottom: S = sJSL.zero
}